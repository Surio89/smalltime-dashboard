<?php
/*MODDED BY SURIO89 - SMALLTIME DASHBOARD EXTENSION*/
/********************************************************************************
* Small Time
/*******************************************************************************
* Version 0.9.011
...
include_once ('./include/class_template_min.php'); 
...
$_template	= new time_template("status.php");
...
switch($_action){
	...
	case "anwesend":
		...
		$_template->_user04 = "sites_login/login_mehr_04_min.php";
		$_template->_user03 = "sites_user/user03_stat.php";
		break;
	case "login_mehr":
	/*
		...
		if($_POST['login'] == "Stempelzeit eintragen" and $_write){
			...
			if($_SESSION['admin']){
				...
				$_logcheck->logout();
				header("Location: status.php?action=login_mehr&tmp=1");
			}else{
				header("Location: status.php?action=login_mehr&tmp=2");
			}
			exit();
		}
